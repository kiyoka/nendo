  ...
  (er-macro-transformer
   (lambda (expr rename compare)
     (let ((ellipse-specified? (identifier? (cadr expr)))
       ...
       (define (next-v)
         (set! count (+ count 1))
         (rename (string->symbol (string-append "v_" (number->string count)))))
...
(define-syntax-ex match-syntax-error
  (syntax-rules ()
...
(define-syntax-ex match-quasiquote
...
(define-syntax-ex match-quasiquote-step
  (syntax-rules ()
...
(define-syntax-ex match-drop-ids
  (syntax-rules ()
    ((_ expr ids ...) expr)))

(define-syntax-ex match-drop-first-arg
  (syntax-rules ()
    ((_ arg expr) expr)))
...
(define-syntax-ex match-gen-or
  (syntax-rules ()
    ((_ v p g+s (sk ...) fk (i ...) ((id id-ls) ...))
     (let ((sk2 (lambda (id ...) (sk ... (i ... id ...)))))
       (match-gen-or-step v p g+s (match-drop-ids (sk2 id ...)) fk (i ...))))))

(define-syntax-ex match-gen-or-step
  (syntax-rules ()
    ((_ v () g+s sk fk . x)
...
(define-syntax-ex match-gen-ellipses
  (syntax-rules ()
    ((_ v p () g+s (sk ...) fk i ((id id-ls) ...))
...
(define-syntax-ex match-verify-no-ellipses
  (syntax-rules ()
    ((_ (x . y) sk)
     (match-check-ellipse
      x
...
(define-syntax-ex match-gen-search
  (syntax-rules ()
...
(define-syntax-ex match-vector-two
  (syntax-rules ()
...
(define-syntax-ex match-vector-step
  (syntax-rules ()
    ((_ v () (sk ...) fk i) (sk ... i))
...
(define-syntax-ex match-gen-vector-ellipses
  (syntax-rules ()
...
(define-syntax-ex match-vector-tail
  (syntax-rules ()
    ((_ v p n len sk fk i)
     (match-extract-vars p (match-vector-tail-two v p n len sk fk i) i ()))))

(define-syntax-ex match-vector-tail-two
  (syntax-rules ()
...
(define-syntax-ex match-extract-vars
...
(define-syntax-ex match-extract-vars-step
  (syntax-rules ()
    ((_ p k i v ((v2 v2-ls) ...))
     (match-extract-vars p k (v2 ... . i) ((v2 v2-ls) ... . v)))
    ))

(define-syntax-ex match-extract-quasiquote-vars
...
(define-syntax-ex match-extract-quasiquote-vars-step
  (syntax-rules ()
...
(define-syntax-ex match-letrec
  (syntax-rules ()
    ((_ vars . body) (match-let/helper letrec () () vars . body))))

(define-syntax-ex match-let/helper
  (syntax-rules ()
    ((_ let ((var expr) ...) () () . body)
     (let ((var expr) ...) . body))
...
(define-syntax-ex match-named-let
  (syntax-rules ()
...
;;   (define-syntax-ex match-check-ellipse
;;     (syntax-rules (...)
;;       ((_ ... sk fk) sk)
;;       ((_ x sk fk) fk)))
...
(define-syntax-ex match-check-identifier
  (syntax-rules ()
